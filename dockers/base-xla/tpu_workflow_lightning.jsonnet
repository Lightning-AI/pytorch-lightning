...
local tputests = base.BaseTest {
  ...
  command: utils.scriptCommand(
    |||
      ...
      python -c "from lightning.fabric.accelerators.tpu import _XLA_AVAILABLE; print(str(_XLA_AVAILABLE))"
      python -c "from lightning.fabric.accelerators import TPUAccelerator; assert TPUAccelerator.is_available()"
      python -c "from lightning.pytorch.accelerators.tpu import _XLA_AVAILABLE; print(str(_XLA_AVAILABLE))"
      python -c "from lightning.pytorch.accelerators import TPUAccelerator; assert TPUAccelerator.is_available()"
      echo "Sanity check passed!"

      cd tests
      cd tests_fabric
      echo "--- Running Fabric tests ---"
      PL_RUN_TPU_TESTS=1 coverage run --source=lightning.pytorch -m pytest -vv --durations=0 ./
      echo "--- Running standalone Fabric tests ---"
      PL_RUN_TPU_TESTS=1 PL_STANDALONE_COVERAGE_SOURCE=lightning PL_STANDALONE_TESTS_BATCH_SIZE=1 bash run_standalone_tests.sh

      cd ..
      cd tests_pytorch
      echo "--- Running PL tests ---"
      PL_RUN_TPU_TESTS=1 coverage run --source=lightning.fabric -m pytest -vv --durations=0 ./
