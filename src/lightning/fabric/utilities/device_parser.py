...
def _parse_gpu_ids(
    ...
    .. note::
        ``include_xpu``, ``include_cuda`` and ``include_mps`` default to ``False`` so that you only
    ...
    gpus = _normalize_parse_gpu_input_to_list(
        gpus, include_xpu=include_xpu, include_cuda=include_cuda, include_mps=include_mps
    )
    if not gpus:
    ...
    if (
        ...
        and len(gpus) != 1
        and len(_get_all_available_gpus(include_xpu=include_xpu, include_cuda=include_cuda, include_mps=include_mps))
        == 1
    ):
    ...
    return _sanitize_gpu_ids(gpus, include_xpu=include_xpu, include_cuda=include_cuda, include_mps=include_mps)
...
def _sanitize_gpu_ids(
    gpus: List[int], include_xpu: bool = False, include_cuda: bool = False, include_mps: bool = False
) -> List[int]:
    ...
    """
    if sum((include_xpu, include_cuda, include_mps)) == 0:
        raise ValueError("At least one gpu type should be specified!")
    all_available_gpus = _get_all_available_gpus(
        include_xpu=include_xpu, include_cuda=include_cuda, include_mps=include_mps
    )
    for gpu in gpus:
...
def _normalize_parse_gpu_input_to_list(
    gpus: Union[int, List[int], Tuple[int, ...]], include_xpu: bool, include_cuda: bool, include_mps: bool
) -> Optional[List[int]]:
    ...
    if not gpus:  # gpus==0
        return None
    if gpus == -1:
        return _get_all_available_gpus(include_xpu=include_xpu, include_cuda=include_cuda, include_mps=include_mps)

    return list(range(gpus))


def _get_all_available_gpus(
    include_xpu: bool = False, include_cuda: bool = False, include_mps: bool = False
) -> List[int]:
    """
    Returns:
    ...
    return xpu_gpus + cuda_gpus + mps_gpus
