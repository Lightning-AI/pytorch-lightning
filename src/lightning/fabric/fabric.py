    ...
    def backward(
        ...
        **kwargs: Any,
        ...
        if not _is_lightning_module:  # cannot use `isinstance` because fabric doesnt install pytorch lightning
            raise RuntimeError(
                "You called `fabric.backward` with a tensor that wasn't generated by Fabric. Did you forget to wrap"
                " your model? `model = fabric.setup(model)`"
            )
        # this path is only used with a `LightningModule` wrapped by Fabric, since the `LightningModule`
        # doesn't return a `_BackwardTensor` from `forward`
