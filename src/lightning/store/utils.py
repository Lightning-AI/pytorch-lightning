...
def _get_model_data(name: str, version: str):
    ...
    if not os.path.exists(_LIGHTNING_STORAGE_FILE):
        raise NotADirectoryError(
            f"Could not find {_LIGHTNING_STORAGE_FILE} (to be generated after download_from_cloud(...))"
        )
    ...
    if username not in storage_data:
        raise KeyError(
            ...
            " `download_from_cloud` before loading"
        )
    ...
    if model_name not in user_data:
        raise KeyError(
            ...
            f" username: {username}. Make sure to call `download_from_cloud` before loading"
        )
