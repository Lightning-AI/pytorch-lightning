...
class PrepareChunksThread(Thread):
    ...
    def __init__(
        ...
        # When the dataset slice fits on the node, we don't need to wait for downloading the chunks.
        if not self._delete_chunks_when_processed:
            self._max_pre_download = 10e7
