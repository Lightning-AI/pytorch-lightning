...
def _collect_artifacts(client: LightningClient, project_id: str, app_id: str, page_token: Optional[str] = "", tokens = None):
    if tokens is None:
        tokens = []
    ...
    for artifact in _collect_artifacts(client, project_id, app_id, page_token=response.next_page_token, tokens=tokens):
        yield artifact