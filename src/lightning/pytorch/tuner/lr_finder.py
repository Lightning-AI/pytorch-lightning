...
# Training loop with learning rate finder
lr_finder = _LRFinder(mode="linear", lr_min=1e-6, lr_max=1, num_training=100)

# Create a learning rate scheduler
lr_lambda = lambda x: lr_finder.lr_min * (lr_finder.lr_max / lr_finder.lr_min) ** (x / lr_finder.num_training)
scheduler = LambdaLR(optimizer, lr_lambda)

lr_finder.results["lr"] = []
lr_finder.results["loss"] = []

for batch_idx, (inputs, targets) in enumerate(dataloader):
    if batch_idx >= lr_finder.num_training:
        break
    ...
    lr_finder.results["lr"].append(scheduler.get_last_lr()[0])
    lr_finder.results["loss"].append(loss.item())
