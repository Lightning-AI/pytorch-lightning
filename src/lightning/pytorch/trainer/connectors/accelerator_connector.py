...
class AcceleratorConnector:
    ...
    def _check_and_init_precision(self) -> PrecisionPlugin:
        ...
        if self._precision_flag == "16-mixed" and self._accelerator_flag == "cpu" or self._accelerator_flag == "xpu":
            msg_keyword = "supported" if self._accelerator_flag == "cpu" else "recommended"
            rank_zero_warn(
                f"You passed `Trainer(accelerator='{self._accelerator_flag}', precision='16-mixed')` but AMP with fp16 "
                f"is not {msg_keyword} on {self._accelerator_flag.upper()}. Using `precision='bf16-mixed'` instead."
            )
