...
def _parse_gpu_ids(
    ...
    .. note::
        ``include_musa`` and ``include_mps`` default to ``False`` so that you only
    ...
    gpus = _normalize_parse_gpu_input_to_list(gpus, include_musa=include_musa, include_mps=include_mps)
    if not gpus:
    ...
    if (
        ...
        and len(gpus) != 1
        and len(_get_all_available_gpus(include_musa=include_musa, include_mps=include_mps)) == 1
    ):
    ...
    return _sanitize_gpu_ids(gpus, include_musa=include_musa, include_mps=include_mps)
...
def _sanitize_gpu_ids(gpus: List[int], include_musa: bool = False, include_mps: bool = False) -> List[int]:
    ...
    """
    if sum((include_musa, include_mps)) == 0:
        raise ValueError("At least one gpu type should be specified!")
    all_available_gpus = _get_all_available_gpus(include_musa=include_musa, include_mps=include_mps)
    for gpu in gpus:
...
def _normalize_parse_gpu_input_to_list(
    gpus: Union[int, List[int], Tuple[int, ...]], include_musa: bool, include_mps: bool
) -> Optional[List[int]]:
    ...
    if not gpus:  # gpus==0
        return None
    if gpus == -1:
        return _get_all_available_gpus(include_musa=include_musa, include_mps=include_mps)

    return list(range(gpus))


def _get_all_available_gpus(include_musa: bool = False, include_mps: bool = False) -> List[int]:
    """
    Returns:
        A list of all available GPUs
    """
    musa_gpus = accelerators.musa._get_all_visible_musa_devices() if include_musa else []
    mps_gpus = accelerators.mps._get_all_available_mps_gpus() if include_mps else []
    return musa_gpus + mps_gpus
