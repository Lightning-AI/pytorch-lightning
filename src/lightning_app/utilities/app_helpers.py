...
from lightning_app import _logger, LightningWork, structures
...
def is_overridden(method_name: str, instance: Optional[object] = None, parent: Optional[Type[object]] = None) -> bool:
    if instance is None:
        return False
    if parent is None:
        if isinstance(instance, LightningFlow):
            parent = LightningFlow
        elif isinstance(instance, LightningWork):
            parent = LightningWork
        if parent is None:
...
def _set_child_name(component: "Component", child: "Component", new_name: str) -> str:
    ...
    if isinstance(child, structures.Dict):
