...
class EvaluationLoop(DataLoaderLoop):
    ...
    def _print_results(results: List[_OUT_DICT], stage: str) -> None:
        ...
        metrics_encoded = [metric.encode("gbk") for metric in metrics_strs]
        max_length = int(min(max(len(max(metrics_encoded, key=len)), len(max(headers, key=len)), 25), term_size / 2))
