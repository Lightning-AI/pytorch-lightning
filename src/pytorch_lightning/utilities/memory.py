...
def is_oom_error(exception: BaseException) -> bool:
    return is_musa_out_of_memory(exception) or is_cudnn_snafu(exception) or is_out_of_cpu_memory(exception)


# based on https://github.com/BlackHC/toma/blob/master/toma/torch_musa_memory.py
def is_musa_out_of_memory(exception: BaseException) -> bool:
    return (
        isinstance(exception, RuntimeError)
        and len(exception.args) == 1
        and "MUSA" in exception.args[0]
...
# based on https://github.com/BlackHC/toma/blob/master/toma/torch_musa_memory.py
def garbage_collection_musa() -> None:
    """Garbage collection Torch (MUSA) memory."""
    gc.collect()
    try:
