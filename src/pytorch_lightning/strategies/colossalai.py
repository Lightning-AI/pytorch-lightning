    ...
    def setup_precision_plugin(self) -> None:
        ...
        if not hasattr(pl_module, "_colossalai_zero"):
            if not _COLOSSALAI_GREATER_0_1_10:
                ...
                search_interval: int = math.ceil(
                    chunk_search_range / self.chunk_size_search_kwargs.get("n_grids", 4096)
                )
                min_chunk_size_mb: float = self.chunk_size_search_kwargs.get("min_chunk_size", 32 * 1024**2)
