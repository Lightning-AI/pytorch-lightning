...
class Timer(Callback):
    ...
    def __init__(
        ...
        self._start_time: Dict[RunningStage, Optional[float]] = {
            stage: None for stage in RunningStage if stage != "tune"
        }
        self._end_time: Dict[RunningStage, Optional[float]] = {stage: None for stage in RunningStage if stage != "tune"}
        self._offset = 0
    ...
    def state_dict(self) -> Dict[str, Any]:
        return {
            "time_elapsed": {stage.value: self.time_elapsed(stage) for stage in list(RunningStage) if stage != "tune"}
        }
