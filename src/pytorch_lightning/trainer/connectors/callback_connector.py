...
def _validate_callbacks_list(callbacks: List[Callback]) -> None:
    ...
    for callback in stateful_callbacks:
        if callback.state_key in seen_callbacks:
            duplicates = seen_callbacks[callback.state_key]
            raise RuntimeError(
                f"Found {len(duplicates)} stateful callbacks of type `{type(callback).__name__}`. In the current"
                ...
                " for the callback to be checkpointable."
            )
