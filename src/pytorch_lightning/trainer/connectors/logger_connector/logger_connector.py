    ...
    def configure_logger(self, logger: Union[bool, Logger, Iterable[Logger]]) -> None:
        if not logger:
            ...
            if _TENSORBOARD_AVAILABLE or _TENSORBOARDX_AVAILABLE:
                ...
                warning_cache.warn(
                    "Starting from release 1.9, `tensorboard` has been removed as a dependency of the `lightning` package, due to potential conflicts with other packages in the ML ecosystem. For this reason, `logger=True` will use `CSVLogger` as the default logger, unless the `tensorboard` package is found. Please `pip install lightning[extra]` to enable TensorBoard support by default".
