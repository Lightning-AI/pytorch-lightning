...
def _train_update(device, epoch, x, loss, tracker, writer):
    test_utils.print_training_update(
        device, x, loss.item(), tracker.rate(), tracker.global_rate(), epoch, summary_writer=writer
    )
...
def train_mnist(flags, fabric, **kwargs):
    ...
    def train_loop_fn(model, loader, epoch):
        ...
        for step, (data, target) in enumerate(loader):
            ...
            if step % flags.log_steps == 0:
                xm.add_step_closure(
                    _train_update, args=(device, epoch, step, loss, tracker, writer), run_async=FLAGS.async_closures
                )
    ...
    for epoch in range(1, flags.num_epochs + 1):
        fabric.print(f"Epoch {epoch} train begin {test_utils.now()}")
        train_loop_fn(model, train_device_loader, epoch)
        fabric.print(f"Epoch {epoch} train end {test_utils.now()}")
        ...
        fabric.print("Epoch {} test end {}, Accuracy={:.2f}".format(epoch, test_utils.now(), accuracy))
    ...
    if flags.ckpt_consolidation:
        ...
        if xm.is_master_ordinal(local=False):
            consolidate_sharded_model_checkpoints(ckpt_prefix=flags.ckpt_prefix, ckpt_suffix="_rank-*-of-*.pth")
    ...
    fabric.print(f"Total training time: {end_time-start_time:.2f}s")
    fabric.print(f"Max Accuracy: {max_accuracy:.2f}%")
    return max_accuracy
...
def main(flags):
    ...
    if flags.auto_wrap_policy != "none":
        if flags.auto_wrap_policy == "size_based":
            ...
            auto_wrap_policy = partial(size_based_auto_wrap_policy, min_num_params=flags.auto_wrap_min_num_params)
            ...
            auto_wrap_policy = partial(transformer_auto_wrap_policy, transformer_layer_cls={nn.Conv2d, nn.Linear})
        else:
            raise Exception(f"Invalid auto-wrap policy: {flags.auto_wrap_policy}")
        if flags.use_gradient_checkpointing:
            # Apply gradient checkpointing to auto-wrapped sub-modules if specified
            auto_wrapper_callable = lambda m, *args, **kwargs: XLAFSDPStrategy(checkpoint_module(m), *args, **kwargs)
