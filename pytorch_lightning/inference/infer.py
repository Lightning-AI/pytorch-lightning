...
class Inference(InferenceABC):
    def __init__(self, network:LightningModule, **kwargs):
...
class TempScalingInference(InferenceABC):
    def __init__(self, network:LightningModule, T:int=1, **kwargs):
        super().__init__(network, **kwargs)
        self.T = 1.5
        if network.val_dataloader(): self._temperature_set(self.network.val_dataloader())
        
    def forward(self, input, T=None):
        T = T if not T else self.T
        return self.infer(input) / T

    def _temperature_set(self, val_dataloader, lr=1e-2, iter=50):
        ...
        optim = torch.optim.LBFGS([T], lr=lr, max_iter=iter)
        def opt():
...
class IterativeInference(InferenceABC):
    def __init__(self, network:LightningModule, N:int=1, infer_mode='train', return_mode='avg', **kwargs):
    ...
    def forward(self, input, N=None):
        ...
        return self.return_mode([self.infer(input) for _ in range(N)])