...
class ModelIO(object):
    ...
    @classmethod
    def _load_model_state(cls, checkpoint: Dict[str, Any], strict: bool = True, **cls_kwargs_new: Any) -> 'ModelIO':
        cls_spec = inspect.getfullargspec(cls.__init__)
        cls_init_args_name = list(inspect.signature(cls.__init__).parameters.keys())

        self_var, args_var, kwargs_var = parse_class_init_keys(cls)
        drop_names = [n for n in (self_var, args_var, kwargs_var) if n]
        cls_init_args_name = [n for n in cls_init_args_name if n not in drop_names]

        cls_kwargs_loaded: Dict = {}
        ...
        if cls.CHECKPOINT_HYPER_PARAMS_KEY in checkpoint:
            ...
            cls_kwargs_loaded = _convert_loaded_hparams(cls_kwargs_loaded, checkpoint[cls.CHECKPOINT_HYPER_PARAMS_TYPE])

            # 4. Update cls_kwargs_new with cls_kwargs_old, such that new has higher priority
            args_name = checkpoint[cls.CHECKPOINT_HYPER_PARAMS_NAME]
        ...
        model = cls(**_cls_kwargs)  # type: ignore

        # give model a chance to load something
        model.on_load_checkpoint(checkpoint)

        # load the state_dict on the model automatically
        model.load_state_dict(checkpoint['state_dict'], strict=strict)  # type: ignore
...
def _convert_loaded_hparams(model_args: dict, hparams_type: Optional[Union[Callable, str]] = None) -> Dict:
...
def load_hparams_from_yaml(config_yaml: str, use_omegaconf: bool = True) -> Dict[str, Any]:
    ...
    with fs.open(config_yaml, "r") as fp:
        hparams = yaml.load(fp, Loader=yaml.UnsafeLoader)  # type: ignore
...
def save_hparams_to_yaml(config_yaml: str, hparams: Union[dict, Namespace]) -> None:
    """
    Args:
