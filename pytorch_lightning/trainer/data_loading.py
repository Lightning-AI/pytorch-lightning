...
def _has_iterable_dataset(dataloader: DataLoader):
    return (
        ITERABLE_DATASET_EXISTS and hasattr(dataloader, 'dataset') and isinstance(dataloader.dataset, IterableDataset)
    )
    ...
    def _worker_check(self, dataloader: DataLoader, name: str) -> None:
        ...
        if is_dataloader and not on_windows:
            if dataloader.num_workers > 0 and using_spawn:
                ...
                rank_zero_warn(
                    'You are using `distributed_backend=ddp_spawn` with num_workers=0.'
