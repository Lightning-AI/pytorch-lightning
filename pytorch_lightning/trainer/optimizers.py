...
class TrainerOptimizersMixin(ABC):
    ...
    def configure_schedulers(self, schedulers: list):
        ...
        default_config = {
            ...
            'monitor': 'val_loss',
        }  # default value to monitor for ReduceLROnPlateau
        for scheduler in schedulers:
            if isinstance(scheduler, dict):
                if 'scheduler' not in scheduler:
                    raise ValueError('Lr scheduler should have key `scheduler`', ' with item being a lr scheduler')
                ...
                lr_schedulers.append({**default_config, 'scheduler': scheduler, 'reduce_on_plateau': True})

            elif isinstance(scheduler, optim.lr_scheduler._LRScheduler):
                lr_schedulers.append({**default_config, 'scheduler': scheduler})
            else:
                raise ValueError(f'Input {scheduler} to lr schedulers ' 'is a invalid input.')
            ...
            for optimizer in optimizers:
                ...
                if scheduler.optimizer == optimizer:
                    ...
                    for i, mro in enumerate(scheduler.__class__.__mro__):
                        if mro == optim.lr_scheduler._LRScheduler or mro == optim.lr_scheduler.ReduceLROnPlateau:
                            idx = i
