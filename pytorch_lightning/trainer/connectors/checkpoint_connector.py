...
class CheckpointConnector:
    ...
    def restore_weights(self, model: LightningModule):
        ...
        if not did_restore_hpc_weights:
            if self.trainer.resume_from_checkpoint is not None:
                self.restore(
                    self.trainer.resume_from_checkpoint, on_gpu=self.trainer.on_gpu
                )
    ...
    def restore(self, checkpoint_path: str, on_gpu: bool):
        ...
        # give the model model a chance to load something
    ...
    def dump_checkpoint(self, weights_only: bool = False) -> dict:
        ...
        if model.hparams:
            if hasattr(model, "_hparams_name"):
                checkpoint[
                    LightningModule.CHECKPOINT_HYPER_PARAMS_NAME
                ] = model._hparams_name
            # add arguments to the checkpoint
            if OMEGACONF_AVAILABLE:
                checkpoint[LightningModule.CHECKPOINT_HYPER_PARAMS_KEY] = model.hparams
                if isinstance(model.hparams, Container):
                    checkpoint[LightningModule.CHECKPOINT_HYPER_PARAMS_TYPE] = type(
                        model.hparams
                    )
            else:
                checkpoint[LightningModule.CHECKPOINT_HYPER_PARAMS_KEY] = dict(
                    model.hparams
                )
