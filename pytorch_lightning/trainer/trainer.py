...
class Trainer(
    ...
    def add_argparse_args(cls, parent_parser: ArgumentParser) -> ArgumentParser:
        r"""Extends existing argparse by default attributes from hierarchy of classes up to `Trainer`.
        ...
        trainer_cls = cls
        while True:
            parser = trainer_cls._add_class_argparse_args(trainer_cls, parser)

            if trainer_cls == Trainer:
                break
            trainer_cls = trainer_cls.__bases__[0]

        return parser

    @staticmethod
    def _add_class_argparse_args(cls, parser: ArgumentParser) -> ArgumentParser:
        r"""Extends existing argparse by default attributes from the `Trainer`'s `cls` subclass.
        """
        blacklist = ['kwargs']
        ...
        for arg, arg_types, arg_default in (
            ...
            parser.add_argument(
                ...
                default=arg_default,
                type=use_type,
                help=f'autogenerated by pl.{cls.__name__}',
                **arg_kwargs,
            )
