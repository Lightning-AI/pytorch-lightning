...
def _auroc_update(preds, target, max_fpr):
...
def _auroc_compute(preds, target, mode, sample_weights, num_classes, average, max_fpr, pos_label) -> torch.Tensor:
    if mode == 'multi-label':
...
def auroc(
    ...
    Example (multiclass case):
        ...
        >>> auroc(preds, target, num_classes=5)
        tensor(0.77)
    
    """
    preds, target, mode = _auroc_update(preds, target, max_fpr)
    return _auroc_compute(preds, target, mode, sample_weights, num_classes, average, max_fpr, pos_label)
