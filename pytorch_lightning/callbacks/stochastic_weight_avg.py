...
class StochasticWeightAveraging(Callback):
    ...
    def on_fit_start(self, trainer: 'pl.Trainer', pl_module: 'pl.LightningModule') -> None:
        ...
        if optimizers is None or len(optimizers) != 1:
    ...
    def on_train_epoch_start(self, trainer: 'pl.Trainer', pl_module: 'pl.LightningModule') -> None:
        if trainer.current_epoch == self.swa_start:
            ...
            optimizers = trainer.optimizers if trainer.optimizers is not None else []
            ...
            if trainer.lr_schedulers:
                ...
                if trainer.lr_schedulers is None:
                    trainer.lr_schedulers = []
                trainer.lr_schedulers.append(default_scheduler_cfg)

            self.n_averaged = torch.tensor(0, dtype=torch.long, device=pl_module.device)

        if self.swa_start <= trainer.current_epoch <= self.swa_end and self._average_model is not None:
        ...
        if trainer.current_epoch == self.swa_end + 1 and self._average_model is not None:
    ...
    def on_train_end(self, trainer: 'pl.Trainer', pl_module: 'pl.LightningModule') -> None:
        if self._model_contains_batch_norm and trainer.current_epoch == self.swa_end + 1:
            ...
            trainer.max_epochs -= 1
            self.reset_momenta()
        elif trainer.current_epoch == self.swa_end and self._average_model is not None:
    ...
    def reset_batch_norm_and_save_state(self, pl_module: 'pl.LightningModule') -> None:
        ...
        for module in pl_module.modules():
            ...
            module.running_mean = torch.zeros_like(
                cast(torch.Tensor, module.running_mean),
                device=pl_module.device,
                dtype=cast(torch.dtype, module.running_mean.dtype)
            )
            module.running_var = torch.ones_like(
                cast(torch.Tensor, module.running_var),
                device=pl_module.device,
    ...
    @staticmethod
    def avg_fn(
        averaged_model_parameter: torch.Tensor, model_parameter: torch.Tensor, num_averaged: torch.Tensor
    ) -> torch.Tensor:
        """
