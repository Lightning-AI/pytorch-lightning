...
def recursive_detach(in_dict: Dict[Any, Union[Tensor, Dict[Any, Tensor]]], to_cpu: bool = False) -> Dict[Any, Tensor]:
...
def is_oom_error(exception) -> bool:
...
def is_cuda_out_of_memory(exception) -> bool:
...
# based on https://github.com/BlackHC/toma/blob/master/toma/torch_cuda_memory.py
def is_cudnn_snafu(exception) -> bool:
...
def is_out_of_cpu_memory(exception) -> bool:
    return isinstance(exception, RuntimeError) \
        and len(exception.args) == 1 \
