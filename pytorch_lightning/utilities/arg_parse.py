...
def add_default_args(parser, root_dir, rand_seed=None, possible_model_names=None):
    ...
    parser.add_argument(
        "--add_log_row_interval", default=100, type=int, help="add log every k batches"
    )

    # early stopping
    parser.add_argument(
        "--disable_early_stop", dest="enable_early_stop", action="store_false"
    )
    parser.add_argument("--early_stop_metric", default="val_acc", type=str)
    parser.add_argument("--early_stop_mode", default="min", type=str)
    parser.add_argument(
        "--early_stop_patience", default=3, type=int, help="number of epochs until stop"
    )
    ...
    if possible_model_names is not None:
        parser.add_argument(
            "--model_name", default="", help=",".join(possible_model_names)
        )
    ...
    parser.add_argument(
        "--tt_save_path", default=root_dir + "/test_tube_logs", help="logging dir"
    )
    ...
    parser.add_argument(
        "--default_tensor_type", default="torch.cuda.FloatTensor", type=str
    )
    ...
    parser.add_argument(
        "--debug", dest="debug", action="store_true", help="enables/disables test tube"
    )
    parser.add_argument(
        "--local", dest="local", action="store_true", help="enables local tng"
    )
