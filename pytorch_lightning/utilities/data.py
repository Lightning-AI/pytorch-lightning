...
def transfer_data_to_device(batch, device_type, idx=None):
    ...
    if isinstance(batch, (list, tuple)):
        ...
        for i, x in enumerate(batch):
            batch[i] = transfer_data_to_device(x, device_type, idx)
        return batch

    # when dict
    if isinstance(batch, dict):
        for k, v in batch.items():
            batch[k] = transfer_data_to_device(v, device_type, idx)
        return batch
