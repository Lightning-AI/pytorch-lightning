...
* ``interval`` (optional): either ``epoch`` (default) for stepping after each training epoch ends, ``step`` for stepping
  after each optimization step or ``val`` for stepping after each time validation is performed (different from 
  choosing ``epoch`` when ``val_check_interval!=1``)
