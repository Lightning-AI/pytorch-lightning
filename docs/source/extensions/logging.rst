...
* ``reduce_fx``: Reduction function over step values for end of epoch. Uses :meth:`torch.mean` by default and is not applied when :class:`torchmetrics.Metric`s are logged.
