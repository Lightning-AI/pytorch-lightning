...
---------------------------------------
Move your optimizers to :func:`pytorch_lightning.core.LightningModule.configure_optimizers` hooks. Make sure to use the hook parameters (self in this case).
...
-----------------------------
Lightning automates the validation (enabling gradients in the train loop and disabling in eval), all you need to keep is the val step logic. To add an (optional) validation loop add logic to :func:`pytorch_lightning.core.LightningModule.validation_step` hook (make sure to use the hook parameters, self in this case):
...
.. testcode::
    ...
    class LitModel(LightningModule):
        ...
        def validation_step(self, batch, batch_idx):
            ...
            val_loss = pl.EvalResult(checkpoint_on=loss)
            return val_loss
...
6. Wrap loss in a TrainResult/EvalResult
----------------------------------------
...
It also allows logging to the progress bar. Read more in :ref:`result`.
...
Here's an example of using the :class:`~pytorch_lightning.trainer.Trainer` with PyTorch Dataloader:
...
Example 1:
==========
Add an MPL layer to finetune an unsupervised learning approach online

Example 2:
==========
Figure out how to tweak an input to trick a network to misslasify a digit


***********************
Step 5: Add A Test Loop
***********************
