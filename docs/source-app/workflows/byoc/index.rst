...
#. Install Lightning.
#. Create a Lightning cluster and then run an App on your cluster.
...
* Terraform (recommended for Terraform users)
...
You're going to create a custom role, that uses a custom policy, in AWS using the AWS management console.

These are the settings you'll specify when creating the role:
...
* **Require external ID:** Provide a secure external ID.
...
If you are an IT person creating this role for someone else (researchers, data scientists, Lightning App creators), assign the role to their AWS account after creating the role.
...
**Who should install Lightning:**: Once your AWS account is ready, IT’s job is done. That means you (researchers, data scientists, Lightning App creators) are the one who will be doing this.
...
**************************************************
Step 3: Set up and use your Lightning BYOC cluster
**************************************************

Creating a cluster is a one time operation, unless you intend to add more clusters.

**Who should set up and use your Lightning BYOC cluster:** With Lightning installed, you (researchers, data scientists, Lightning App creators, and more) can now start creating clusters and running Apps on your clusters.
...
Verify the status of your Lightning BYOC clusters
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Start an App on your Lightning BYOC cluster
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Once your cluster is running, you can run any Lightning App on your cluster. To run an App on your Lightning BYOC cluster, use **--cloud** and **–-cluster <cluster name>** in the command:
