...
and is completely agnostic to :meth:`~lightning_pytorch.trainer.trainer.Trainer.fit` call. The logic used here is defined under
...
You can still run inference on a test dataset even if the :meth:`~lightning_pytorch.core.hooks.DataHooks.test_dataloader` method hasn't been
...
The logic associated to the validation is defined within the :meth:`~lightning_pytorch.core.LightningModule.validation_step`.

Apart from this ``.validate`` has same API as ``.test``, but would rely respectively on :meth:`~lightning_pytorch.core.LightningModule.validation_step` and :meth:`~lightning_pytorch.core.LightningModule.test_step`.

.. note::
    ``.validate`` method uses the same validation logic being used under validation happening within
    :meth:`~lightning_pytorch.trainer.trainer.Trainer.fit` call.
