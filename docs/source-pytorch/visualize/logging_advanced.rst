...
****************************
To change the default values (ie: version number) shown in the progress bar, override the :meth:`~lightning_pytorch.callbacks.progress.progress_bar.ProgressBar.get_metrics` method in your logger.

.. code-block:: python

    from lightning_pytorch.callbacks.progress import Tqdm
...
To change this behaviour, set the *log_every_n_steps* :class:`~lightning_pytorch.trainer.trainer.Trainer` flag.
...
Send logs to the logger like ``Tensorboard``, or any other custom logger passed to the :class:`~lightning_pytorch.trainer.trainer.Trainer` (Default: ``True``).
