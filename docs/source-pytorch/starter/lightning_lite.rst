...
:class:`~pytorch_lightning.lite.Fabric` enables pure PyTorch users to scale their existing code
...
:class:`~pytorch_lightning.lite.Fabric` is the right tool for you if you match one of the two following descriptions:
...
.. warning:: :class:`~pytorch_lightning.lite.Fabric` is currently a beta feature. Its API is subject to change based on your feedback.
...
1. Create the :class:`~pytorch_lightning.lite.Fabric` object at the beginning of your training code.
2. Remove all ``.to`` and ``.cuda`` calls since :class:`~pytorch_lightning.lite.Fabric` will take care of it.
3. Apply :meth:`~pytorch_lightning.lite.Fabric.setup` over each model and optimizers pair and :meth:`~pytorch_lightning.lite.Fabric.setup_dataloaders` on all your dataloaders and replace ``loss.backward()`` by ``lite.backward(loss)``.
4. Run the script from the terminal using ``lightning run model path/to/train.py`` or use the :meth:`~pytorch_lightning.lite.Fabric.launch` method in a notebook.
...
:class:`~pytorch_lightning.lite.Fabric` can also figure it out automatically for you!
...
If you require custom data or model device placement, you can deactivate :class:`~pytorch_lightning.lite.Fabric`'s automatic placement by doing ``lite.setup_dataloaders(..., move_to_device=False)`` for the data and ``lite.setup(..., move_to_device=False)`` for the model.
Furthermore, you can access the current device from ``lite.device`` or rely on :meth:`~pytorch_lightning.lite.Fabric.to_device` utility to move an object to the current device.
...
The :class:`~pytorch_lightning.lite.Fabric` provides you with the tools to scale your training, but there are several major challenges ahead of you now:
...
If you are facing one of those challenges, then you are already meeting the limit of :class:`~pytorch_lightning.lite.Fabric`.
