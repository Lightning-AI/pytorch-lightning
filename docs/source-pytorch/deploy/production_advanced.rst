...
To export your model to ONNX format call the :meth:`~lightning_pytorch.core.LightningModule.to_onnx` function on your :class:`~lightning_pytorch.core.LightningModule` with the ``filepath`` and ``input_sample``.
...
You can also skip passing the input sample if the ``example_input_array`` property is specified in your :class:`~lightning_pytorch.core.LightningModule`.
...
In order to do so, your LightningModule needs to subclass the :class:`~lightning_pytorch.serve.servable_module.ServableModule`, implements its hooks and pass a :class:`~lightning_pytorch.serve.servable_module_validator.ServableModuleValidator` callback to the Trainer.
