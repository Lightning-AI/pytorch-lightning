...
def test_lightning_cli_config_and_subclass_mode(tmpdir):
    input_config = {
        "fit": {
            "model": {"class_path": "unittests_pl.helpers.BoringModel"},
            "data": {"class_path": "unittests_pl.helpers.BoringDataModule", "init_args": {"data_dir": str(tmpdir)}},
...
def test_lightning_cli_help():
    ...
    cli_args = ["any.py", "fit", "--data.help=unittests_pl.helpers.BoringDataModule"]
    out = StringIO()
    with mock.patch("sys.argv", cli_args), redirect_stdout(out), pytest.raises(SystemExit):
        any_model_any_data_cli()
...
def test_lightning_cli_print_config():
    cli_args = [
        ...
        "predict",
        "--seed_everything=1234",
        "--model=unittests_pl.helpers.BoringModel",
        "--data=unittests_pl.helpers.BoringDataModule",
        "--print_config",
    ]
    out = StringIO()
    with mock.patch("sys.argv", cli_args), redirect_stdout(out), pytest.raises(SystemExit):
...
def test_lightning_cli_link_arguments(tmpdir):
    ...
    cli_args[-1] = "--model=unittests_pl.utilities.test_cli.BoringModelRequiredClasses"
