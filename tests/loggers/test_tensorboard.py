...
def test_tensorboard_log_sub_dir(tmpdir):
    ...
    assert trainer.logger.experiment_dir == os.path.join(save_dir, "name", "version")

    # sub_dir specified
    logger = TestLogger(save_dir, sub_dir="sub_dir")
    trainer = Trainer(**trainer_args, logger=logger)
    assert trainer.logger.experiment_dir == os.path.join(save_dir, "name", "version", "sub_dir")
    ...
    logger = TestLogger(save_dir, sub_dir="sub_dir")
    trainer = Trainer(**trainer_args, logger=logger)
    assert trainer.logger.experiment_dir == os.path.join(explicit_save_dir, "name", "version", "sub_dir")
