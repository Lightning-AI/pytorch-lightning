...
from pytorch_lightning.loggers import TensorBoardLogger, MLFlowLogger, NeptuneLogger, TestTubeLogger, CometLogger
...
def test_loggers_fit_test(tmpdir, monkeypatch, logger_class):
    ...
    trainer = Trainer(max_epochs=1, logger=logger, train_percent_check=0.2, val_percent_check=0.5, fast_dev_run=True,)
    trainer.fit(model)
...
def test_logger_reset_correctly(tmpdir, extra_params):
    ...
    trainer = Trainer(default_save_path=tmpdir, **extra_params)
