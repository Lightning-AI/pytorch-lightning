...
def test_accelerator_choice_ddp_cpu(tmpdir, num_processes: int, num_nodes: int):
    ...
    assert isinstance(trainer.training_type_plugin, DDPStrategy if no_spawn else DDPSpawnStrategy)
...
@RunIf(min_gpus=2)
@pytest.mark.parametrize(
    ["devices", "plugin"], [(1, SingleDevicePlugin), ([1], SingleDeviceStrategy), (2, DDPSpawnStrategy)]
)
