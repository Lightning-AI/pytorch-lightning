...
class DeterministicModel(LightningModule):
    ...
    def training_epoch_end_return_for_log_epoch_and_step(self, result):
        ...
        result.log(
            'epoch_end_log_acc',
            torch.tensor(1212).type_as(result.step_epoch_log_acc2_epoch),
            logger=True,
            on_epoch=True,
        )
        result.log(
            'epoch_end_pbar_acc',
            torch.tensor(1213).type_as(result.step_epoch_log_acc2_epoch),
            logger=False,
            prog_bar=True,
            on_epoch=True,
        )
        result.log(
            ...
            torch.tensor(1214).type_as(result.step_epoch_log_acc2_epoch),
            logger=True,
            prog_bar=True,
            on_epoch=True,
        )
