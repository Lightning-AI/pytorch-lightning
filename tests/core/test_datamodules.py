...
def test_data_hooks_called():
    dm = BoringDataModule()
    assert not dm._has_prepared_data
    assert not dm._has_setup_fit
    assert not dm._has_setup_test
    assert not dm._has_setup_validate
    assert not dm._has_setup_predict
    assert not dm._has_teardown_fit
    assert not dm._has_teardown_test
    assert not dm._has_teardown_validate
    assert not dm._has_teardown_predict

    dm.prepare_data()
    assert dm._has_prepared_data
    assert not dm._has_setup_fit
    assert not dm._has_setup_test
    assert not dm._has_setup_validate
    assert not dm._has_setup_predict
    ...
    dm.setup()
    assert dm._has_prepared_data
    assert dm._has_setup_fit
    assert dm._has_setup_test
    assert dm._has_setup_validate
    assert not dm._has_setup_predict
    assert not dm._has_teardown_fit
    assert not dm._has_teardown_test
    assert not dm._has_teardown_validate
    ...
    assert dm._has_setup_validate
    assert not dm._has_setup_predict
    assert dm._has_teardown_fit
    assert dm._has_teardown_test
    assert dm._has_teardown_validate
    assert not dm._has_teardown_predict
...
def test_data_hooks_called_verbose(use_kwarg):
    ...
    assert dm._has_teardown_validate
    assert dm._has_teardown_test
    assert dm._has_teardown_predict
