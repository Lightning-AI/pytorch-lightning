...
class BaseParityAutomatiocOptimizationModel(BoringModel):
    ...
    def __init__(self, optimizer_name, accumulated_grad_batches=None, mocked=False):
        super().__init__()
...
class AutomatiocOptimizationNativeModel(BaseParityAutomatiocOptimizationModel):
    ...
    def optimizer_step(self, current_epoch, batch_nb, optimizer, optimizer_idx, closure,
        ...
        closure()
        if should_accumulate(self.trainer, self.accumulated_grad_batches):
            return
...
# TODO: Add parity test for ddp, hovorod, apex, accumulated_grad_batches > 1, multiple optimizers
...
def test_parity_training_lightning_optimizer_one_optimizer(tmpdir, amp_backend, precision, accumulate_grad_batches):
    ...
    def parity_train_with_one_optimizer(enable_pl_optimizer, vanilla=False, mock=False):
        seed_everything(42)
        if vanilla:
            # Note: global_step counts training_loop.optimizer_step
            expected_global_step = expected_batches
            if using_amp:
                model = AutomatiocOptimizationVanillaAMPNativeModel("SGD")
            else:
                model = AutomatiocOptimizationVanillaAMPNativeModel("SGD")
            ...
            if enable_pl_optimizer:
                model = BaseParityAutomatiocOptimizationModel("Adam" if mock else "SGD")
                initial_weights["model_wi_pl_optimizer"] = model.layer.weight.clone()
            else:
                model = BaseParityAutomatiocOptimizationModel("AdamW" if mock else "SGD")
    ...
    model_wo_pl_optimizer = parity_train_with_one_optimizer(using_amp, expected_batches, False)
    model_wi_pl_optimizer = parity_train_with_one_optimizer(using_amp, expected_batches, True)
    ...
    assert model_wo_pl_optimizer.on_model_wo_pl_optimizer_zero_grad_count == model_wi_pl_optimizer.on_model_wo_pl_optimizer_zero_grad_count
    ...
    override = parity_train_with_one_optimizer(using_amp, expected_batches, False, override=True)

    assert torch.equal(initial_weights["model_wo_pl_optimizer"], initial_weights["override"])
    assert override.grad_checked
    assert override.losses == model_wo_pl_optimizer.losses
    assert (override.on_model_wo_pl_optimizer_zero_grad_count // accumulate_grad_batches) == model_wo_pl_optimizer.on_model_wo_pl_optimizer_zero_grad_count

    for b_grad, o_grad in zip(model_wo_pl_optimizer.grads, override.grads):
        assert torch.equal(b_grad, o_grad), 'Grad parameters are different'

    for b_w, o_w in zip(model_wo_pl_optimizer.parameters(), override.parameters()):
    ...
    if precision == 32:
        ...
        with patch("torch.optim.SGD.step") as mock_sdg_step, \
            ...
            model_wo_pl_optimizer = parity_train_with_one_optimizer(using_amp, expected_batches, False, mock=True)
            model_wi_pl_optimizer = parity_train_with_one_optimizer(using_amp, expected_batches, True, mock=True)
            override = parity_train_with_one_optimizer(using_amp, expected_batches, False, override=True, mock=True)
