...
@RunIf(min_gpus=2, special=True)
def test_ddp_terminate_when_deadlock_is_detected_oom(tmpdir):
    """Test that DDP kills the remaining processes when only one rank is throwing an OOM exception."""

    class TestModel(BoringModel):
        def train_dataloader(self):
            # Try to force the following error:
            ...
            # RandomDataset(32, 10000000) is approximately 1.28GB.
            return DataLoader(RandomDataset(32, 10000000), num_workers=7, batch_size=64)
