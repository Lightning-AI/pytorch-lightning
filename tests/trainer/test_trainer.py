...
def test_exception_when_testing_or_validating_with_fast_dev_run(tmpdir):
    ...
    with pytest.raises(MisconfigurationException, match="trainer.validate\(\)` with `fast_dev_run=True"):
        trainer.validate()
    with pytest.raises(MisconfigurationException, match="trainer.test\(\)` with `fast_dev_run=True"):
        trainer.test()
