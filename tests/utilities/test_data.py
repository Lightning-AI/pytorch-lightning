...
def test_extract_batch_size():
    ...
    with pytest.warns(UserWarning, match="Lightning is trying to infer the `batch_size`. We found 11."):
        extract_batch_size(batch)

    batch = {"test": [{"test": [torch.zeros(10, 10), torch.zeros(11, 10)]}]}
    with pytest.warns(UserWarning, match="Lightning is trying to infer the `batch_size`. We found 10."):
        extract_batch_size(batch)
