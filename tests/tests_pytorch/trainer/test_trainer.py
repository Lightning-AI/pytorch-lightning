...
def test_no_val_module(monkeypatch, tmp_path, tmp_path_server, url_ckpt):
    ...
    ckpt_path = (
        f"http://{tmp_path_server[0]}:{tmp_path_server[1]}/{os.path.basename(new_weights_path)}"
        if url_ckpt
...
@pytest.mark.parametrize("url_ckpt", [True, False])
def test_strict_model_load(monkeypatch, tmp_path, tmp_path_server, url_ckpt):
    """Tests use case where trainer saves the model, and user loads it from tags independently."""
    # set $TORCH_HOME, which determines torch hub's cache path, to tmp_path
...
@pytest.mark.parametrize("url_ckpt", [True, False])
def test_fit_ckpt_path_epoch_restored(monkeypatch, tmp_path, tmp_path_server, url_ckpt):
...
def test_checkpoint_path_input(tmp_path, ckpt_path, save_top_k, fn):
    ...
    if ckpt_path == "best":
        ...
        if save_top_k == 0:
            ...
            ckpt_path = str(
                list((tmp_path / f"lightning_logs/version_{trainer.logger.version}/checkpoints").iterdir())[
                    0
                ].absolute()
            )
            trainer_fn(ckpt_path=ckpt_path)
