...
def test_lightning_env_parse(cleandir):
    ...
    assert cli.config.fit.trainer.logger is None


def test_lightning_cli_save_config_cases(cleandir):

    config_path = "config.yaml"
    cli_args = ["fit", "--trainer.logger=null", "--trainer.fast_dev_run=1"]
...
def test_lightning_cli_save_config_only_once(cleandir):
    config_path = "config.yaml"
    cli_args = ["--trainer.logger=null", "--trainer.max_epochs=1"]
...
def test_lightning_cli_config_and_subclass_mode(cleandir):
    input_config = {
        "fit": {
            ...
            "trainer": {"max_epochs": 1, "enable_model_summary": False},
        }
    }
    config_path = "config.yaml"
...
def test_cli_config_overwrite(cleandir):
    trainer_defaults = {"max_steps": 1, "max_epochs": 1}
...
def test_cli_logger_shorthand():
    with mock.patch("sys.argv", ["any.py"]):
        cli = LightningCLI(TestModel, run=False, trainer_defaults={"logger": None})
    ...
    with mock.patch("sys.argv", ["any.py", "--trainer.logger=null"]):
        cli = LightningCLI(TestModel, run=False)
    assert cli.trainer.logger is None
...
@pytest.mark.parametrize(
    ["args"],
    [
        (["--trainer.logger=null", "--model.foo=456"],),
        ({"trainer": {"logger": None}, "model": {"foo": 456}},),
        (Namespace(trainer=Namespace(logger=None), model=Namespace(foo=456)),),
    ],
)
def test_lightning_cli_with_args_given(args):
    ...
    assert isinstance(cli.model, TestModel)
    assert cli.config.trainer.logger is None
