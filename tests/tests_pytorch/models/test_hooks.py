...
def test_trainer_model_hook_system_fit(tmp_path, kwargs, automatic_optimization):
    called = []
...
def test_trainer_model_hook_system_eval(tmp_path, batches, verb, noun, dataloader, key):
    called = []
    model = HookedModel(called)
    callback = HookedCallback(called)
    trainer = Trainer(
...
def test_load_from_checkpoint_hook_calls(tmp_path):
