...
def test_top_k(save_mock, tmpdir, k: int, epochs: int, val_check_interval: float, expected: int, save_last: bool):
    ...
    model = TestModel()
    trainer = Trainer(
        callbacks=[ModelCheckpoint(dirpath=tmpdir, monitor="my_loss", save_top_k=k, save_last=save_last)],
        default_root_dir=tmpdir,
        max_epochs=epochs,
        enable_model_summary=False,
        val_check_interval=val_check_interval,
...
def test_top_k_ddp(save_mock, tmpdir, k, epochs, val_check_interval, expected):
    class TestModel(BoringModel):
        def training_step(self, batch, batch_idx):
            self.log("my_loss", batch_idx * (1 + self.trainer.local_rank), on_epoch=True)
            return super().training_step(batch, batch_idx)

        def on_train_epoch_end(self):
            if self.trainer.is_global_zero:
                self.log("rank_zero_only_loss", (1 + self.trainer.local_rank), on_epoch=True, rank_zero_only=True)
