...
def test_mlflow_logger_exists(client, _, tmp_path):
    ...
    logger = MLFlowLogger("test", save_dir=tmp_path)
    ...
    logger2 = MLFlowLogger("test", save_dir=tmp_path)
    ...
    logger3 = MLFlowLogger("new", save_dir=tmp_path)
...
@mock.patch("lightning.pytorch.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
@mock.patch("lightning.pytorch.loggers.mlflow.MlflowClient")
def test_mlflow_run_name_setting(client, _, tmp_path):
    ...
    logger = MLFlowLogger("test", run_name="run-name-1", save_dir=tmp_path)
    ...
    logger = MLFlowLogger("test", run_name="run-name-1", tags={MLFLOW_RUN_NAME: "run-name-2"}, save_dir=tmp_path)
    logger = mock_mlflow_run_creation(logger, experiment_id="exp-id")
    _ = logger.experiment
    client.return_value.create_run.assert_called_with(experiment_id="exp-id", tags=tags)
...
@mock.patch("lightning.pytorch.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
@mock.patch("lightning.pytorch.loggers.mlflow.MlflowClient")
def test_mlflow_run_id_setting(client, _, tmp_path):
    ...
    logger = MLFlowLogger("test", run_id=run.info.run_id, save_dir=tmp_path)
    _ = logger.experiment
...
@mock.patch("lightning.pytorch.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
@mock.patch("lightning.pytorch.loggers.mlflow.MlflowClient")
def test_mlflow_log_dir(client, _, tmp_path):
    ...
    trainer.fit(model)
    assert trainer.checkpoint_callback.dirpath == (tmp_path / "exp-id" / "run-id" / "checkpoints")
...
@mock.patch("lightning.pytorch.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
@mock.patch("lightning.pytorch.loggers.mlflow.MlflowClient")
def test_mlflow_experiment_id_retrieved_once(client, tmp_path):
...
@mock.patch("lightning.pytorch.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
@mock.patch("lightning.pytorch.loggers.mlflow.MlflowClient")
def test_mlflow_logger_with_unexpected_characters(client, _, __, tmp_path):
...
@mock.patch("lightning.pytorch.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
@mock.patch("lightning.pytorch.loggers.mlflow.MlflowClient")
def test_mlflow_logger_experiment_calls(client, _, time, param, metric, tmp_path):
    """Test that the logger calls methods on the mlflow experiment correctly."""
    time.return_value = 1

    logger = MLFlowLogger("test", save_dir=tmp_path, artifact_location="my_artifact_location")
...
@mock.patch("lightning.pytorch.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
@mock.patch("lightning.pytorch.loggers.mlflow.MlflowClient")
def test_mlflow_logger_with_long_param_value(client, _, param, tmp_path):
...
@mock.patch("lightning.pytorch.loggers.mlflow._MLFLOW_AVAILABLE", return_value=True)
@mock.patch("lightning.pytorch.loggers.mlflow.MlflowClient")
def test_mlflow_logger_with_many_params(client, _, param, tmp_path):
...
def test_mlflow_log_model(client, _, tmp_path, log_model):
    """Test that the logger creates the folders and files in the right place."""
    # Get model, logger, trainer and train
    model = BoringModel()
    logger = MLFlowLogger("test", save_dir=tmp_path, log_model=log_model)
