...
def test_restore_checkpoint_after_pre_setup(tmp_path, restore_after_pre_setup):
    ...
    class TestPlugin(SingleDeviceStrategy):
        ...
        def load_checkpoint(self, checkpoint_path: Union[str, Path], weights_only: bool, state: Any) -> dict[str, Any]:
            assert self.setup_called == restore_after_pre_setup
            return super().load_checkpoint(checkpoint_path, weights_only, state=state)
