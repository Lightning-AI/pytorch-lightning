...
def test_lightningCLI_seed_everything_default_to_None_deprecation_warning():
    with mock.patch("sys.argv", ["any.py"]), pytest.warns(FutureWarning)(
...
def test_deprecated_dataloader_reset():
    trainer = Trainer()
    with pytest.warns(FutureWarning)(match="reset_train_val_dataloaders` has been deprecated in v1.7"):
