...
from pytorch_lightning.profiler import SimpleProfiler, AdvancedProfiler, HPUProfiler
...
class TestHPUProfiler:
    ...
    def test_hpu_simple_profiler_instances(self, tmpdir, get_device_count):
        model = BoringModel()
        trainer = Trainer(profiler="simple", accelerator="hpu", devices=get_device_count, max_epochs=1, default_root_dir=tmpdir)
    ...
    def test_hpu_simple_profiler_trainer_stages(self, tmpdir, get_device_count):
        ...
        trainer = Trainer(profiler=profiler, accelerator="hpu", devices=get_device_count, max_epochs=1, default_root_dir=tmpdir)
    ...
    def test_hpu_advanced_profiler_instances(self, tmpdir, get_device_count):
        model = BoringModel()
        trainer = Trainer(profiler="advanced", accelerator="hpu", devices=get_device_count, max_epochs=1, default_root_dir=tmpdir)
    ...
    def test_simple_profiler_distributed_files(self, tmpdir, get_device_count):
        ...
        expected = {f"{stage}-profiler-{rank}.txt" for stage in ("fit", "validate", "test") for rank in range(0, trainer.devices)}
        actual = set(os.listdir(profiler.dirpath))
    ...
    def test_hpu_trace_event_cpu_instant_event(tmpdir):
        ...
        devices = habana_frameworks.torch.hpu.device_count()
        model = BoringModel()
        trainer = Trainer(accelerator="hpu", devices=devices, max_epochs=1,
                          profiler=HPUProfiler(profile_memory=True))
        trainer.fit(model)
        assert trainer.state.finished, f"Training failed with {trainer.state}"
    ...
    def test_hpu_trace_event_python_function(tmpdir):
        ...
        trainer = Trainer(accelerator="hpu", devices=devices, max_epochs=1,
                          profiler=HPUProfiler(with_stack=True))
        trainer.fit(model)
        assert trainer.state.finished, f"Training failed with {trainer.state}"
    ...
    def test_hpu_trace_event_cpu_op_input_dim(tmpdir):
        ...
        trainer = Trainer(accelerator="hpu", devices=devices, max_epochs=1,
                          profiler=HPUProfiler(record_shapes=True))
        trainer.fit(model)
        assert trainer.state.finished, f"Training failed with {trainer.state}"
