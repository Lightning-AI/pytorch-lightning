...
import pytest
import torch
from skimage.metrics import peak_signal_noise_ratio as ski_psnr, structural_similarity as ski_ssim
...
from pytorch_lightning.metrics.functional import mae, mse, psnr, rmse, rmsle, ssim


@pytest.mark.parametrize(
    ['sklearn_metric', 'torch_metric'],
    [
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param([0.0, 1, 2, 3], [0.0, 1, 2, 2], 0.25),
        pytest.param([4.0, 3, 2, 1], [1.0, 4, 3, 2], 3.0),
    ],
)
...
@pytest.mark.parametrize(
    ['pred', 'target', 'expected'],
    [
        pytest.param([0.0, 1, 2, 3], [0.0, 1, 2, 3], 0.0),
        pytest.param([0.0, 1, 2, 3], [0.0, 1, 2, 2], 0.5),
        pytest.param([4.0, 3, 2, 1], [1.0, 4, 3, 2], 1.7321),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param([0.0, 1, 2, 3], [0.0, 1, 2, 3], 0.0),
        pytest.param([0.0, 1, 2, 3], [0.0, 1, 2, 2], 0.25),
        pytest.param([4.0, 3, 2, 1], [1.0, 4, 3, 2], 1.5),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param([0.0, 1, 2, 3], [0.0, 1, 2, 3], 0.0),
        pytest.param([0.0, 1, 2, 3], [0.0, 1, 2, 2], 0.1438),
        pytest.param([4.0, 3, 2, 1], [1.0, 4, 3, 2], 0.5330),
    ],
)
...
@pytest.mark.parametrize(
    ['pred', 'target'],
    [
        pytest.param([0.0, 1.0, 2.0, 3.0], [0.0, 1.0, 2.0, 3.0]),
        pytest.param([0.0, 1.0, 2.0, 3.0], [0.0, 1.0, 2.0, 2.0]),
        pytest.param([4.0, 3.0, 2.0, 1.0], [1.0, 4.0, 3.0, 2.0]),
    ],
)
def test_psnr_with_skimage(pred, target):
    score = psnr(pred=torch.tensor(pred), target=torch.tensor(target), data_range=3)
...
def test_psnr_base_e_wider_range(pred, target):
    score = psnr(pred=torch.tensor(pred), target=torch.tensor(target), data_range=4, base=2.718281828459045)
...
@pytest.mark.parametrize(
    ['sklearn_metric', 'torch_metric'], [pytest.param(ski_psnr, psnr, id='peak_signal_noise_ratio')]
)
def test_psnr_against_sklearn(sklearn_metric, torch_metric):
    ...
    for n_cls_pred, n_cls_target in [(10, 10), (5, 10), (10, 5)]:
        ...
        sk_score = sklearn_metric(target.cpu().detach().numpy(), pred.cpu().detach().numpy(), data_range=n_cls_target)
        sk_score = torch.tensor(sk_score, dtype=torch.float, device=device)
...
@pytest.mark.parametrize(
    ['size', 'channel', 'coef', 'multichannel'],
    [
