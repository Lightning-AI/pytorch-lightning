...
def test_model(clean_home, pbar, model_name: str = "boring_model", version: str = "latest"):
    ...
    assert os.path.isdir(os.path.join(clean_home, __STORAGE_DIR_NAME, _USERNAME, model_name, version))

    model = load_model(f"{_USERNAME}/{model_name}")
    assert model is not None


def test_only_weights(clean_home, model_name: str = "boring_model_only_weights", version: str = "latest"):
    model = BoringModel()
...
def test_checkpoint_path(clean_home, model_name: str = "boring_model_only_checkpoint_path", version: str = "latest"):
    model = BoringModel()
    trainer = pl.Trainer(fast_dev_run=True)
    trainer.fit(model)
