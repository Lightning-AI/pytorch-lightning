...
def test_loop_state_on_exception(accumulate_grad_batches, stop_epoch, stop_batch, stop_optimizer, n_optimizers, tmpdir):
    ...
    # need to remove these elements for comparison; comparing with `fit_loop.state_dict()` would require the fit loop
    # to have an iterator, which is only available during training
    checkpoint["loops"]["fit_loop"]["state_dict"]["dataloader_state_dict"] = {}
